package com.dgu.clubauth.domain.club.entity.mapping;
...
@Getter
@Table(name = "club_student") // DB의 테이블 정의
@EntityListeners(AuditingEntityListener.class)
public class ClubStudent extends BaseEntity {
    @Id // DB의 Primary Key
    ...
    @Enumerated(EnumType.STRING) // Enum을 사용 시 데이터 형태 명시
    @Builder.Default
    private Role role = Role.MEMBER; // 직책

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "club_id", nullable = false)
    private Club club; // 동아리 아이디
